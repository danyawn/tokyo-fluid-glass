...
const InfiniteScroll: React.FC<InfiniteScrollProps> = ({
  ...
  useEffect(() => {
    ...
    const measuredHeight = firstItem.offsetHeight || itemMinHeight;
    const itemStyle = getComputedStyle(firstItem);
    const itemMarginTop = parseFloat(itemStyle.marginTop) || 0;
    const totalItemHeight = measuredHeight + itemMarginTop;
    const totalHeight = measuredHeight * items.length + itemMarginTop * (items.length - 1);
    ...
    if (autoplay) {
      ...
      if (pauseOnHover) {
        const stopTicker = () => { if (rafId) cancelAnimationFrame(rafId); };
